...
function darkenColor(hex: string, percent: number) {
  ...
  const red = Math.max(
    0,
    Math.min(255, Math.floor(((value >> 16) & 255) * (1 - percent))),
  );
  const green = Math.max(
    0,
    Math.min(255, Math.floor(((value >> 8) & 255) * (1 - percent))),
  );
  const blue = Math.max(
    0,
    Math.min(255, Math.floor((value & 255) * (1 - percent))),
  );
...
export default function Folder({
  ...
  return (
    <div className={`folder-wrap ${className}`.trim()} style={folderStyle}>
      <div
        className={`folder${open ? " folder--open" : ""}${papers.length === 0 ? " folder--empty" : ""}`}
        data-count={papers.length}
      >
        <span className="folder__back">
          {papers.map((item, index) => {
            const itemLabel =
              isValidElement<{ alt?: string }>(item) &&
              typeof item.props.alt === "string"
                ? item.props.alt
                : `certificate ${index + 1}`;
